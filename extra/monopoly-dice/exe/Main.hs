...
{- Note: Choosing to do even a small part of a Haskell project using unbounded 
   ...
   as a parameter or return an Int as a value. It is my understanding that the reason
   there are both Int and an Integer type classes is that it is slightly more 
...
{- Using the reverse-chunked [[Integer]] generated by the above function, it is 
   ...
   comparing each "chunk". The number of potential events is the number of 
   elements in all of the chunks minus the one representing the first revolution, 
   while the number of events is the number of elements in each chunk (after the
   one representing the first) which also appear in the previous chunk:  -}
